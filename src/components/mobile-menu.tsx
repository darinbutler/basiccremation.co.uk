...
export function MobileMenu() {
  ...
  // We can only use createPortal after the component has mounted on the client
  ...
  const overlay = open ? (
    ...
    >
      {/* Backdrop — covers full viewport, click to close */}
      <button
        type="button"
        onClick={close}
      ...
      <div className="absolute right-0 top-0 bottom-0 w-[88vw] max-w-sm bg-paper shadow-cardHover overflow-y-auto animate-slide-in-right">
        {/* Top bar */}
        ...
        <nav className="px-2 py-3" aria-label="Mobile navigation">
          ...
          <div className="mt-2 px-3 py-2 border-t border-ink-100">
            ...
            <Link
              ...
              onClick={close}
            >
              View all 73 locations &rarr;
            </Link>
          </div>
        </nav>

        {/* Bottom padding so last item isn't flush with bottom of panel */}
  ...
  return (
    <>
      ...
      {/* Portal the overlay directly to document.body so it escapes the sticky
          header's stacking context — this was the bug: menu was visually stuck
          inside the header's z-40 box and couldn't cover the page below. */}
